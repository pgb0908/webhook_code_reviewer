...
| **Push 코드리뷰** | MR에 새 커밋 push(`update`) | 증분 diff만 분석해 MR 코멘트로 게시 |
...
GitLab Webhook
 ...
 FastAPI Server (POST /webhook)
      │
      ├─ MR open ──► sync → diff(전체) → overview 생성 → MR 제목/설명 교체
      │
      ├─ MR update(push) ──► sync → 증분 diff(oldrev..HEAD) → overview 생성 → MR 코멘트 게시
      │
...
│   └── tasks.py             # overview / comment / push-review 파이프라인
├── git/
│   ├── sync.py              # 저장소 clone/pull
│   └── diff.py              # diff 추출, 청크 분할, 언어 감지
├── gitlab/
...
- Aider는 repo map을 통해 워크스페이스 전체 구조를 스스로 파악합니다. `--file` 인자를 별도로 지정하지 않습니다.
